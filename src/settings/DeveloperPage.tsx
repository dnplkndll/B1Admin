...
import { Box, Table, TableHead, TableRow, TableCell, TableBody, IconButton, Tooltip, Button, Typography } from "@mui/material";
import { Key as KeyIcon, Delete as DeleteIcon, Add as AddIcon, Link as LinkIcon } from "@mui/icons-material";
  ...
  return (
    <>
      <PageHeader title={Locale.label("settings.developer.title")} subtitle={Locale.label("settings.developer.subtitle")}>
        {!showKeyEdit && (
          <Button variant="contained" startIcon={<AddIcon />} onClick={() => setShowKeyEdit(true)} sx={{ color: "#FFF", backgroundColor: "rgba(255,255,255,0.2)", borderColor: "#FFF", "&:hover": { backgroundColor: "rgba(255,255,255,0.3)" } }}>
      ...
      <Box sx={{ p: 2 }}>
        {showKeyEdit ? (
          ...
          <>
            <Typography variant="h6" sx={{ mb: 1 }}>{Locale.label("settings.developer.apiKeys")}</Typography>
            {apiKeys.length === 0 ? (
              <Box sx={{ textAlign: "center", py: 4 }}>
                <KeyIcon sx={{ fontSize: 40, color: "text.secondary", mb: 1 }} />
            ...
            <Typography variant="h6" sx={{ mt: 4, mb: 1 }}>{Locale.label("settings.developer.connectedApps")}</Typography>
