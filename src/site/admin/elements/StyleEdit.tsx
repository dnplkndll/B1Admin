...
export const StyleEdit: React.FC<Props> = (props) => {
  ...
  return <FormCard onSave={handleSave} saveText={Locale.label("common.update")} title={Locale.label("site.style.editStyle")} onCancel={() => { props.onSave("", "", ""); }} onDelete={(props.style.name) ? () => { props.onSave(props.style.platform, props.style.name, null ); } : undefined}>
